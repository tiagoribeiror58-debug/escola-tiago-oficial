...
import { Loader2, BookOpen, X, Sparkles, CheckCircle } from 'lucide-react';
...
export default function ReflectionModal({ materiaSlug, topico, initialValue = '', onComplete, onCancel }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data?.note?.ai_complement) {
        setAiFeedback(data.note.ai_complement);
      } else {
      ...
      <div className="bg-card w-full max-w-lg rounded-2xl shadow-2xl border border-border overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="px-5 py-4 border-b border-border flex items-center justify-between bg-muted/30 shrink-0">
          <div className="flex items-center gap-2">
            <div className="p-2 bg-primary/10 rounded-lg">
              {aiFeedback ? <Sparkles className="w-5 h-5 text-primary" /> : <BookOpen className="w-5 h-5 text-primary" />}
            </div>
            <div>
              <h2 className="font-semibold text-foreground">
                {aiFeedback ? 'Feedback da IA' : (isChatNote ? 'Salvar Nota de Estudo' : 'O que você aprendeu?')}
              </h2>
              <p className="text-xs text-muted-foreground">
                {aiFeedback ? 'Revisão concluída' : (isChatNote ? 'Revisão com IA' : 'Técnica de Feynman (Opcional)')}
              </p>
            </div>
          </div>
          {onCancel && !isSubmitting && !aiFeedback && (
        ...
        <div className="px-5 py-4 border-t border-border bg-muted/10 flex flex-col-reverse sm:flex-row items-center justify-between gap-3 shrink-0">
          <button
            ...
            disabled={isSubmitting || (!aiFeedback && !reflection.trim())}
            className={cn(
              "py-3 sm:py-2 bg-primary text-primary-foreground text-sm font-medium rounded-xl hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2 shadow-md",
              isChatNote || aiFeedback ? "w-full" : "w-full sm:w-auto px-6",
              (isSubmitting || (!aiFeedback && !reflection.trim())) && "opacity-50 pointer-events-none"
            )}
          >
            {isSubmitting ? (
              <>
